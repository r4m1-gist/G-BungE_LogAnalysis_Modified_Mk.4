...
현재 웹 기반 베타 버전도 함께 올려 두었습니다. CLI 옵션을 직접 입력하지 않고
브라우저에서 로그와 분석 항목을 선택해 같은 분석 코드를 실행할 수 있습니다.
...
웹 기반 베타 버전은 `web/server.py`로 실행합니다.

브라우저에서 로그 그룹, 로그 파일, 분석 플롯을 선택해 CLI 분석을 실행하려면
로컬 웹 서버를 켭니다.
...
웹 서버는 기존 `main.py`, `logFetcher.py`, `logPostProcessor.py`를 그대로
사용하고, Matplotlib 결과를 `web_runs/` 아래 PNG로 저장해 브라우저에
보여줍니다. 생성된 결과와 Matplotlib 캐시는 Git에 포함하지 않습니다.

`web/index.html`은 GitHub Pages 같은 정적 호스팅용 안내 페이지입니다.
정적 페이지에서는 Python 분석 코드를 실행할 수 없으므로, 실제 로그 분석은
위의 로컬 서버로 실행해야 합니다.
